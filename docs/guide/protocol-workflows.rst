...
**slug** (for example ``my-protocol-slug``). Hosted submission uses that slug with
JSON inputs — not the local YAML file. You can also execute a *Local Protocol
Profile*–compatible YAML on your machine with ``biolm protocol run-local``
(requires ``biolm[pipeline]``); see :doc:`../notes/local-protocol-profile-v1`.
Validate YAML locally, then either run it locally or submit inputs against the
registered slug.
